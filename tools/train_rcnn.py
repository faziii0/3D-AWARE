...
parser.add_argument("--teacher_cfg_file", type=str, default=None, help="path to teacher YAML config file")
...
if __name__ == "__main__":
    ...
    # Load the teacher model and checkpoint
    if args.teacher_cfg_file:
        cfg_from_file(args.teacher_cfg_file)
    teacher_model = PointRCNN(num_classes = train_loader.dataset.num_class, use_xyz = True, mode='TEST')
    teacher_model.cuda()
    teacher_ckpt = torch.load(args.teacher_ckpt)
    teacher_model.load_state_dict(teacher_ckpt['model_state'], strict=False)
    #print(teacher_model)
    teacher_model.eval()
    
    # Restore student configuration
    cfg_from_file(args.cfg_file)
