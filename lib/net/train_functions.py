...
def Feature_Gather(feature_map, xy):
    ...
    interpolate_feature = grid_sample(feature_map, xy, align_corners=True)  # Fixes the warning

    return interpolate_feature.squeeze(2) # (B,C,N)

def test_teacher_student_outputs(student_output, teacher_output):
    print("\n[TEST] Verifying teacher and student outputs...\n")
    expected_keys = ['rpn_cls', 'rpn_reg', 'rcnn_cls', 'rcnn_reg']
    passed = True

    for key in expected_keys:
        s_val = student_output.get(key)
        t_val = teacher_output.get(key)
        ...
        if s_val.shape != t_val.shape:
            print(f"[FAIL] Shape mismatch for key: {key} -> Student: {s_val.shape}, Teacher: {t_val.shape}")
            passed = False
            continue

        if torch.isnan(s_val).any() or torch.isnan(t_val).any():
            print(f"[FAIL] NaNs found in key: {key}")
            passed = False

        if torch.isinf(s_val).any() or torch.isinf(t_val).any():
            print(f"[FAIL] Infs found in key: {key}")
            passed = False

        print(f"[PASS] {key} shape: {s_val.shape}")
    print("Student keys:", list(student_output.keys()))
    print("Teacher keys:", list(teacher_output.keys()))
    if passed:
        print("\n✅ All checks passed! Teacher and student outputs are valid.\n")
    else:
        print("\n❌ Found issues with the outputs. Please fix before distillation.\n")
...
def model_joint_fn_decorator():
    ModelReturn = namedtuple("ModelReturn", ['loss', 'tb_dict', 'disp_dict','ret_dict','teacher_ret_dict'])
    MEAN_SIZE = torch.from_numpy(cfg.CLS_MEAN_SIZE[0]).cuda()

    def model_fn(model, teacher_model, data):
        if cfg.RPN.ENABLED:
        ...
        for k in input_data:
            if isinstance(input_data[k], torch.Tensor):
                input_data[k] = input_data[k].to(device, non_blocking=True).contiguous()
        ret_dict = model(input_data)
        #print("ret_dict keys:", ret_dict.keys())
       
        # Forward pass for the teacher model
        #teacher_ret_dict = teacher_model(input_data)

        with torch.no_grad():
            teacher_ret_dict = teacher_model(input_data)

        # ✅ Add the call here:
        print("[DEBUG] Running teacher/student output test...")


        test_teacher_student_outputs(ret_dict, teacher_ret_dict)
        ...
        return ModelReturn(loss, tb_dict, disp_dict,ret_dict,teacher_ret_dict)
