...
def grid_sample_reverse(point_feature, xy, img_shape):
    '''
    :param point_feature: (B,C,N)
    :param xy: (B,N,2) --> [-1,1]
    :param img_shape: [B,C,H,W]
    :return:
    '''
...
class KittiRCNNDataset(KittiDataset):
    ...
    def get_rpn_with_li_fusion(self, index):
        ...
        if sample_id < 10000:
            ...
            if self.split == "train" or self.split == "val":
                img_seg_mask = self.get_KINS_car_mask(sample_id)  #######addd this shi

            img_seg_mask = self.get_KINS_car_mask(sample_id)

            if cfg.USE_PAINTING_SCORE:
                painting_score_map = self.get_painting_score_lidar(sample_id)

            if cfg.USE_PAINTING_FEAT:
                painting_feat_map = self.get_painting_feat_lidar(sample_id)

            if cfg.USE_PSEUDO_LIDAR:
                #print('###################USE_PSEUDO_LIDAR ')
                pts_pseudo_lidar = self.get_pseudo_lidar(sample_id)
                pts_pseudo_rect = calib.lidar_to_rect(pts_pseudo_lidar[:, 0:3])
                pts_pseudo_intensity = np.zeros_like(pts_pseudo_lidar[:, 0])
        ...
        if cfg.USE_PSEUDO_LIDAR:
            pts_pseudo_img, pts_rect_pseudo_depth = calib.rect_to_img(pts_pseudo_rect)
            pts_valid_flag = self.get_valid_flag(pts_pseudo_rect, pts_pseudo_img, pts_rect_pseudo_depth, img_shape)

            pts_pseudo_rect = pts_pseudo_rect[pts_valid_flag][:, 0:3]
            pts_pseudo_intensity = pts_pseudo_intensity[pts_valid_flag]
            pts_pseudo_origin_xy = pts_pseudo_img[pts_valid_flag]

            select_points_num = pts_rect.shape[0]
            all_pseudo_choice = np.arange(0, len(pts_pseudo_rect), dtype=np.int32)
            all_pseudo_choice = np.random.choice(all_pseudo_choice, select_points_num, replace=False)

            pts_pseudo_rect = pts_pseudo_rect[all_pseudo_choice, :]
            pts_pseudo_intensity = pts_pseudo_intensity[all_pseudo_choice]
            pts_pseudo_origin_xy = pts_pseudo_origin_xy[all_pseudo_choice,:]

            ### concat pseudo and real lidar point:
            pts_rect = np.concatenate((pts_rect, pts_pseudo_rect), axis=0)
            pts_intensity = np.concatenate((pts_intensity, pts_pseudo_intensity), axis=0)
            pts_origin_xy = np.concatenate((pts_origin_xy, pts_pseudo_origin_xy), axis=0)

        if cfg.USE_PAINTING_SCORE:
            pts_paint_scores = painting_score_map[pts_origin_xy[:,1].astype(int), pts_origin_xy[:,0].astype(int), :]
            pts_paint_ori_scores = sigmoid_func(pts_paint_scores)
            #pts_rect = np.concatenate((pts_rect, pts_paint_scores), axis=-1)

        if cfg.USE_PAINTING_FEAT:
            pts_paint_ori_feats = painting_feat_map[pts_origin_xy[:,1].astype(int), pts_origin_xy[:,0].astype(int), :]
            #pts_rect = np.concatenate((pts_rect, pts_paint_feats), axis=-1)
        
        # print(f"img.shape")
        # if cfg.USE_IM_DEPTH: ## shape: 384, 1280, 4
        #     # img[pts_origin_xy[:,1].astype(int), pts_origin_xy[:,0].astype(int), 3] = pts_rect_depth[pts_valid_flag] / 100.0  ### 除以100是由于输入的depth也除以了100进行归一化
        #     # depth = pts_rect_depth[pts_valid_flag] / 100.0 
        #     depth = 
            
        
            ### 除以100是由于输入的depth也除以了100进行归一化
        ...
        if self.mode == 'TRAIN' or self.random_select:
            ...
            if self.npoints < len(pts_rect):
                ...
                if self.npoints > 4096:
                    ...
                    all_choice = np.arange(0, len(pts_rect), dtype = np.int32)
                    choice = np.random.choice(all_choice, self.npoints, replace=False)
                #########################################    zliu smaple 2020.10.20
            else:
            ...
            if cfg.USE_PAINTING_SCORE:
                pts_paint_scores = pts_paint_ori_scores[choice, :]
        ...
        # print('#################################')
        # print('sample_id:', sample_id)
        # print('ret_pts_origin_xy:', ret_pts_origin_xy.shape)
        # print('ret_pts_rect:', ret_pts_rect.shape)
        # #assert ret_pts_origin_xy.shape[0]==2048
        # print('#################################')
        # if cfg.INPUT_CROSS_FUSION:  ERROR!!
        #     cross_input_feat = np.concatenate([ret_pts_rect, ret_pts_features], axis=1)
        #     xy_cor_ori = ret_pts_origin_xy.copy()
        #     size_range = [1280.0, 384.0]
        #     x_cor_ori = xy_cor_ori[:, 0, np.newaxis] / (size_range[0] - 1.0) * 2.0 - 1.0
        #     y_cor_ori = xy_cor_ori[:, 1, np.newaxis] / (size_range[1] - 1.0) * 2.0 - 1.0
        #     xy_cor_concat = np.concatenate([x_cor_ori, y_cor_ori],axis=-1)
        #     cross_input_feat_torch = torch.from_numpy(cross_input_feat).unsqueeze(0).permute(0, 2, 1)
        #     xy_cor_concat_torch = torch.from_numpy(xy_cor_concat).unsqueeze(0)
        #     project_point2img_feat = grid_sample_reverse(cross_input_feat_torch, xy_cor_concat_torch, img_shape=img.shape)

        if cfg.INPUT_CROSS_FUSION: ## shape: 384, 1280, 4
            point2img = np.zeros([384, 1280, 4], dtype=np.float)
            point2img[ret_pts_origin_xy[:, 1].astype(int), ret_pts_origin_xy[:, 0].astype(int), 0:3] = ret_pts_rect / (np.abs(ret_pts_rect).max()+0.0001)
            point2img[ret_pts_origin_xy[:, 1].astype(int), ret_pts_origin_xy[:, 0].astype(int), 3] = ret_pts_intensity  ### 除以100是由于输入的depth也除以了100进行归一化

            ##print('########img.shape:', np.concatenate([img, point2img],axis=-1).shape)

            sample_info = {'sample_id': sample_id, 'random_select': self.random_select, 'img': np.concatenate([img, point2img], axis=-1),
                           'pts_origin_xy': ret_pts_origin_xy}

        else:
            if cfg.USE_PAINTING_SCORE:
                sample_info = {'sample_id': sample_id, 'random_select': self.random_select, 'img': img,
                               'pts_origin_xy': ret_pts_origin_xy, 'pts_paint_scores': pts_paint_scores}
            elif cfg.USE_PAINTING_FEAT:
                sample_info = {'sample_id': sample_id, 'random_select': self.random_select, 'img': img,
                               'pts_origin_xy': ret_pts_origin_xy, 'pts_paint_feats': pts_paint_feats}
            else:
                sample_info = {'sample_id': sample_id, 'random_select': self.random_select, 'img': img,
                               'pts_origin_xy': ret_pts_origin_xy}
        ...
        if self.mode == 'TEST':
            ...
            if cfg.USE_IM_DEPTH:       ###############addd shi
                sample_info['depth'] = self.get_img_depth(sample_id)
        
        
            if cfg.USE_SEPARATE_DEPTH_INPUT:
                if cfg.USE_SINGLE_CHANNEL_DEPTH_INPUT:    ###### single channel depth input
                    # print("############################## single channel depth input #############################")
                    sample_info['depth'] = self.get_img_depth(sample_id)
                elif cfg.USE_MONO_DEPTH_INPUT:           ##### multidepth  channel depth input, maybe of multi channel
                    sample_info['depth'] = self.get_img_mono_depth(sample_id)
            return sample_info
        ...
        if cfg.USE_IMAGE_LOSS:
            sample_info['img_seg_mask'] = img_seg_mask
        # if cfg.USE_IMAGE_LOSS:
        #     rpn_image_seg_label = np.zeros([384, 1280, 1], dtype=np.float)
        #     rpn_image_seg_label[ret_pts_origin_xy[:, 1].astype(int), ret_pts_origin_xy[:, 0].astype(int), 0] = rpn_cls_label
        #     sample_info['rpn_image_seg_label'] = rpn_image_seg_label
        return sample_info
    ...
    def get_proposal_from_file(self, index):
        ...
        if cfg.RCNN.ROI_SAMPLE_JIT:
            ...
            sample_dict['depth'] = self.get_img_depth(sample_id)  ######adddd
            if cfg.USE_IM_DEPTH:
                sample_dict['depth'] = self.get_img_depth(sample_id)
            ...
            if cfg.USE_SEPARATE_DEPTH_INPUT:
                if cfg.USE_SINGLE_CHANNEL_DEPTH_INPUT:    ###### single channel depth input
                    ...
                    sample_dict['depth'] = self.get_img_mono_depth(sample_id)
        ...
        for key in batch[0].keys():
            ...
            if isinstance(batch[0][key], np.ndarray):
                if batch_size == 1:
                    ...
                    #     print('key', key)
                    #     print('batch[k][key]:', batch[kk][key].shape)
