...
class KittiDataset(torch_data.Dataset):
    def __init__(self, root_dir, split = 'train'):
        ...
        self.mono_depth_dir = os.path.join(self.imageset_dir, 'mono_depth')
        if cfg.USE_IM_DEPTH:
    ...
    def get_image_rgb_with_normal(self, idx):
        ...
        # ##################################
        if cfg.USE_IM_DEPTH:
            imback = np.zeros([384, 1280, 4], dtype=np.float64)  ##  imback = np.zeros([384, 1280, 4], dtype=np.float)
    ...
    def get_img_depth(self, idx):
        # Your existing code
        depth_file = os.path.join(self.depth_dir, '%06d.png' % idx)
        depth = np.array(Image.open(depth_file)).astype(np.float32)
        depth = depth / 256.0

        # Converting depth values to the range (0, 100)
        depth = depth * 100.0
        depth_back = np.zeros([384, 1280], dtype=np.float64)  
        depth_back[:depth.shape[0], :depth.shape[1]] = depth
        
        return depth_back
    
    
    def get_img_mono_depth(self, idx):
        # Your existing code
        depth_file = os.path.join(self.mono_depth_dir, '%06d.png' % idx)
        depth = np.array(Image.open(depth_file)).astype(np.float32)
        depth = depth / 256.0
        ...
        depth_back = np.zeros([384, 1280,3], dtype=np.float64)  
        depth_back[:depth.shape[0], :depth.shape[1], :] = depth
